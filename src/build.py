...
splits some relations into several rows and drops the types it has no rule for.
    ...
    python3 src/build.py --pbf japan-260831.osm.pbf
"""
import argparse
import collections
import json
...
EXTRACT = os.environ.get("EXTRACT", os.path.join(BASE, "tmp/japan-260831.osm.pbf"))
...
def osm_names_and_counts(pbf):
    ...
    A brand that appears in the counts and not in the names carries
    brand:wikidata on a feature with no name key at all. It has no spelling,
    so it has nothing to say in a file about spellings and gets no record.
    main() says how many, because a number silently missing from a total is
    how the first version of this dataset went wrong.
    """
...
def main():
    ...
    silent = set(counts) - set(names)
    print(f"{len(rows)} brands, {sum(r['features'] for r in rows):,} features")
    if silent:
        print(f"{len(silent)} more brands carry brand:wikidata on "
              f"{sum(counts[q] for q in silent)} features with no name key at "
              f"all, and are not in the file")
